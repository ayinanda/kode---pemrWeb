...
public function getTenureStats() {
    $sql = "SELECT
                CASE
                    WHEN hire_date >= CURRENT_DATE - INTERVAL '1 year' THEN 'Junior'
                    WHEN hire_date >= CURRENT_DATE - INTERVAL '3 years' THEN 'Middle'
                    ELSE 'Senior'
                END AS masa_kerja,
...
public function getEmployeeOverview() {
    $sql = "select 
	count(*) as total_karyawan,
	sum (salary) as total_gaji_per_bulan,
	avg (extract(year from age(current_date,hire_date))) as rata_rata_masa_kerja
    from employees";
